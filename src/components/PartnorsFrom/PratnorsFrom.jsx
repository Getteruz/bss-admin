...
export default function PratnorsFrom() {
    const params = useParams()
    const [tag, setTags] = useState(data?.job)
    const [name, setName] = useState(data?.name)
    return (
        <div className='ServicesFrom'>
            <div className="ServicesFrom_top">
                <button className='ServicesFrom_top-back'><Link className='ServicesFrom_top-back2' to={routes.PARTNORS}>Добавление сотрудники
                </Link></button>
            ...
            <form className="ServicesFrom_from">
                <div className='ServicesFrom_from-mid mid2'>
                    ...
                    <div className='ServicesFrom_from-mid-left'>
                        <input className='ServicesFrom_from-mid-inputtitle inputtitle2' value={name} type="text" placeholder='Имя фамилия' onClick={(e) => e.target.classList.add("inputtagcolor")} onChange={(e) => {
